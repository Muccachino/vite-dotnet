...
export async function getSingleCourse(id: string) {
  const course = await axios.get(`https://localhost:44395/api/courses/${id}`);
  return course.data
}

export async function getCourseTitle(courseID: number) {
  const response = await axios.get(`https://localhost:44395/api/Courses`);
...
export default function useCourses(){
  ...
  const editCourses = async (courseToEdit: ICourse) => {
    ...
    const response: ICourse = await axios.put(`https://localhost:44395/api/courses/${courseToEdit.courseID}`, courseToEdit);
  ...
  const createCourse = async (courseToAdd: ICourse) => {
    const response = await axios.post(`https://localhost:44395/api/Courses`, courseToAdd);
    setCourses((prevCourses) => [...prevCourses, response.data]);
  }

  const deleteCourse = async (courseToDelete: ICourse) => {
    const response = await axios.delete(`https://localhost:44395/api/Courses/${courseToDelete.courseID}`);
    if(response.status === 200){
      setCourses((prevCourses) =>
